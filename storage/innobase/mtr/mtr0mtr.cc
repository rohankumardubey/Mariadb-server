...
void mtr_t::rollback_to_savepoint(ulint savepoint)
{
  ...
  while (s-- > savepoint)
  {
    ...
    if (!slot.object)
      continue;
    /* This is intended for releasing latches on buffer pages that
    have not been modified by the mini-transaction. */
...
void mtr_t::memo_release(const void *object, mtr_memo_type_t type)
{
  ut_ad(is_active());
  ut_ad(m_memo);
...
struct FlaggedCheck {
	...
	bool operator()(const mtr_memo_slot_t* slot) const
	{
		...
		if (f & (MTR_MEMO_PAGE_S_FIX | MTR_MEMO_PAGE_SX_FIX
			 | MTR_MEMO_PAGE_X_FIX)) {
			block_lock* lock = &static_cast<buf_page_t*>(
